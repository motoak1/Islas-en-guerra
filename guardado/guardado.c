...
void menuPausaDibujar(HDC hdcBuffer, RECT rect, MenuPausa *menu) {
  ...
  // Fuentes
  HFONT fontTitulo =
      CreateFontA(28, 0, 0, 0, FW_BOLD, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
  ...
  case MODO_CARGAR:
    ...
    break;
  case MODO_CONFIRMAR_SALIR:
    titulo = "SALIR AL MENU";
    break;
  }
  ...
  if (menu->modo == MODO_PRINCIPAL) {
    ...
    for (int i = 0; i < OPCIONES_PAUSA_TOTAL; i++) {
      COLORREF color =
          (i == menu->seleccion) ? RGB(100, 255, 120) : RGB(200, 200, 200);
    ...
    TextOutA(hdcBuffer, panelX + (panelAncho - iSize.cx) / 2, inicioY, instruc,
             strlen(instruc));
    ...
    HPEN campoBorde =
        CreatePen(PS_SOLID, 2,
                  menu->nombreExiste ? RGB(255, 100, 100) : RGB(100, 150, 200));
    HPEN oldP = (HPEN)SelectObject(hdcBuffer, campoBorde);
    HBRUSH nullB = (HBRUSH)GetStockObject(NULL_BRUSH);
    HBRUSH oldB = (HBRUSH)SelectObject(hdcBuffer, nullB);
    Rectangle(hdcBuffer, campoX, campoY, campoX + campoAncho,
              campoY + campoAlto);
    SelectObject(hdcBuffer, oldP);
    SelectObject(hdcBuffer, oldB);
    ...
    TextOutA(hdcBuffer, campoX + 10, campoY + 8, textoConCursor,
             strlen(textoConCursor));
    ...
    if (menu->nombreExiste) {
      ...
      TextOutA(hdcBuffer, panelX + (panelAncho - errSize.cx) / 2, campoY + 45,
               err, strlen(err));
      SelectObject(hdcBuffer, fontOpciones);
    }
    ...
    TextOutA(hdcBuffer, panelX + panelAncho / 4 - gSize.cx / 2, btnY,
             btnGuardar, strlen(btnGuardar));

    SetTextColor(hdcBuffer, RGB(255, 200, 100));
    SIZE vSize;
    GetTextExtentPoint32A(hdcBuffer, btnVolver, strlen(btnVolver), &vSize);
    TextOutA(hdcBuffer, panelX + 3 * panelAncho / 4 - vSize.cx / 2, btnY,
             btnVolver, strlen(btnVolver));
    ...
    if (menu->numPartidas == 0) {
      ...
      TextOutA(hdcBuffer, panelX + (panelAncho - nhSize.cx) / 2, inicioY + 50,
               noHay, strlen(noHay));
    } else {
      for (int i = 0; i < menu->numPartidas && i < 5; i++) {
        COLORREF color = (i == menu->partidaSeleccionada) ? RGB(100, 255, 120)
                                                          : RGB(200, 200, 200);
        SetTextColor(hdcBuffer, color);

        char linea[128];
        snprintf(linea, sizeof(linea), "%s - Isla %d (%s)",
                 menu->partidas[i].nombreJugador, menu->partidas[i].islaActual,
    ...
    TextOutA(hdcBuffer, panelX + (panelAncho - vSize.cx) / 2, btnY, volver,
             strlen(volver));
    ...
    TextOutA(hdcBuffer, panelX + (panelAncho - aSize.cx) / 2, inicioY,
             advertencia, strlen(advertencia));

    SetTextColor(hdcBuffer, RGB(200, 200, 200));
    const char *pregunta = "Seguro que deseas salir?";
    SIZE pSize;
    GetTextExtentPoint32A(hdcBuffer, pregunta, strlen(pregunta), &pSize);
    TextOutA(hdcBuffer, panelX + (panelAncho - pSize.cx) / 2, inicioY + 30,
             pregunta, strlen(pregunta));

    // Opciones
    int btnY = inicioY + 80;
    const char *opSi =
        menu->seleccion == 0 ? "> Salir sin guardar <" : "Salir sin guardar";
    const char *opNo =
        menu->seleccion == 1 ? "> Volver y guardar <" : "Volver y guardar";

    SetTextColor(hdcBuffer, menu->seleccion == 0 ? RGB(255, 100, 100)
                                                 : RGB(180, 180, 180));
    SIZE siSize;
    GetTextExtentPoint32A(hdcBuffer, opSi, strlen(opSi), &siSize);
    TextOutA(hdcBuffer, panelX + (panelAncho - siSize.cx) / 2, btnY, opSi,
             strlen(opSi));

    SetTextColor(hdcBuffer, menu->seleccion == 1 ? RGB(100, 255, 120)
                                                 : RGB(180, 180, 180));
    SIZE noSize;
    GetTextExtentPoint32A(hdcBuffer, opNo, strlen(opNo), &noSize);
    TextOutA(hdcBuffer, panelX + (panelAncho - noSize.cx) / 2, btnY + 35, opNo,
             strlen(opNo));
  }
  ...
  if (menu->timerMensaje > 0 && menu->mensaje[0] != '\0') {
    ...
    SIZE msgSize;
    GetTextExtentPoint32A(hdcBuffer, menu->mensaje, strlen(menu->mensaje),
                          &msgSize);
...
bool menuPausaProcesarTecla(MenuPausa *menu, WPARAM tecla, struct Jugador *j,
    ...
    if (tecla == VK_RETURN) {
      if (menu->cursorPos > 0 && !menu->nombreExiste) {
        ...
        if (guardarPartidaPorNombre(menu->nombreInput, j, cam)) {
          snprintf(menu->mensaje, sizeof(menu->mensaje),
                   "Partida guardada correctamente!");
          obtenerRutaGuardado(menu->nombreInput, menu->rutaGuardado,
                              sizeof(menu->rutaGuardado));
  ...
  if (tecla == VK_UP || tecla == 'W' || tecla == 'w') {
    if (menu->modo == MODO_PRINCIPAL) {
      menu->seleccion =
          (menu->seleccion - 1 + OPCIONES_PAUSA_TOTAL) % OPCIONES_PAUSA_TOTAL;
    } else if (menu->modo == MODO_CARGAR && menu->numPartidas > 0) {
      menu->partidaSeleccionada =
          (menu->partidaSeleccionada - 1 + menu->numPartidas) %
          menu->numPartidas;
    } else if (menu->modo == MODO_CONFIRMAR_SALIR) {
  ...
  if (tecla == VK_RETURN) {
    if (menu->modo == MODO_PRINCIPAL) {
      ...
      if (menu->numPartidas > 0) {
        const char *nombre =
            menu->partidas[menu->partidaSeleccionada].nombreJugador;
        if (cargarPartidaPorNombre(nombre, j, cam, ayuntamiento, mina,
                                   cuartel)) {
